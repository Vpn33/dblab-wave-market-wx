...
module.exports = {
    ...
    readImportWave: function (path) {
        // 读取导入的波形文件
        if (!path) {
            console.error("called readImportWave failed, path is required!");
            return;
        }
        let res = new Promise((resolve, reject) => {
    ...
    getChannelData(channel) {
        ...
        if (channel && enable) {
            ...
            if (stageArr && stageArr.length > 0) {
                ...
                _.merge(_.last(data), waveStart);
                _.merge(_.last(data), waveEnd);
            }
        }
        return data;
    },
    ...
    arrayBuffer2Binary(buf) { // 二进制字符串转arrayBuffer 转二进制字符串
