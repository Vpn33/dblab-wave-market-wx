...
module.exports = {
    ...
    analyzeWave: function (wave) {
        ...
        if (wave && wave.id && wave.name) {
            // 单通道
            if (wave.stages && wave.stages.length > 0) {
    ...
    getWaveData(hz, z) {
        ...
        // Y = Frequency - X
        let x = parseInt(Math.pow(hz / 1000, 0.5) * 15);
        // X的范围是【0-31】
        if (x < 0) {
            x = 0;
        } else {
