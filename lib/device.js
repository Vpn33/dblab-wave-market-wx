...
function runWorker(channel, invWorker, wave, songChannel, dataIdx) {
    ...
    let play = function (param) {
        ...
        // 如果不是编辑器模式且达到最小播放时长 就播放下一首 单位:秒
        // 如果设置了预备下一曲标记也需要检查是否完整播放
        if (isEditing === false && invTotal > 0 && ((invTotal % players[channel].minTime === 0) || param.preNext == true)) {
...
function playNext(channel) {
    ...
    switch (type) {
        ...
        case '3':
            // 单曲循环不用改变
            let wave = playList[idx];
            // 如果单曲循环的是双通道的波形 就需要重新触发tooglePlay
            if (wave.channelType === '1') {
                togglePlay(channel, idx);
            }
            // 不是双通道的不用处理 worker会直接进入下次循环
            return;
    }
