...
function setAipwInfo(channel) {
    ...
    if (!aiPwRec[param.waveId]) {
        aiPwRec[param.waveId] = {};
    }
    aiPwRec[param.waveId][param.stageIdx] = players[channel].pw;
 ...
 * 
 */
function delAipwInfo(waveId) {
    _.unset(aiPwRec, waveId);
    wa.writeAiPw(aiPwRec);
}
...
function togglePlay(channel, playIdx) {
    ...
    if (_.isNumber(playIdx)) {
        ...
        if (invWorker.isRunning) {
            ...
            let waveTotal = (param.wavePlayCnt * invWorker.invTime) / 1000;
            // 如果是双通道的波形
            ...
            invWorker.clean();
            // 记录播放时长次数等信息
            recordPlayCnt(param.waveId, waveTotal);
            return;
        }
    }
...
function runWorker(channel, invWorker, wave, songChannel, dataIdx) {
    ...
    let play = function (param) {
        ...
        // 已执行的时间 单位:秒  this.invCnt + this.invTime是因为到达这里已经执行过一次了 因为invCnt是在worker中累加的 所以此处要比worker中少一次
        let invTotal = ((this.invCnt + 1) * this.invTime) / 1000;
        // 当前波形已经播放的时间
        let waveTotal = (param.wavePlayCnt * this.invTime) / 1000;
        // 如果处于暂停播放状态
        if (param.pause === true) {
        ...
        if (isEditing === false && param.pause !== true && waveTotal > 0 && ((waveTotal % players[channel].minTime === 0) || param.preNext == true)) {
 ...
 * @param {*} waveTotal 
 */
function recordPlayCnt(waveId, waveTotal) {
    // 大于1秒的才进行记录
    if (waveId && waveTotal && waveTotal >= 1) {
        (async () => {
            // 读取波形
            ...
            // 播放次数+1 
            let cnt = wave.playCnt || 0;
            cnt += 1;
            wave.playCnt = cnt;

            // 播放时长 + playTolTime
            let tolTime = wave.playTolTime || 0;
            tolTime += waveTotal;
            wave.playTolTime = tolTime;
            if (wave) {
                wa.writeWave(wave);
            }
        })();
...
function anlySongStart(song, param, channel) {
    if (song.type) {
        ...
        if (song.type === 'start') {
            ...
            // 手动设置的电源>0
            if (pw > 0) {
                // 如果小节开始且执行总时间大于5分钟就开始记录智能强度
                let invTotal = (param.invCnt * param.invTime) / 1000;
                // 300秒(5分钟)以后记录
                if (invTotal > 300 && players[channel].aiPw === true) {
                    // 记录智能强度
                    setAipwInfo(channel);
                }
                // 如果有电源增量
                ...
                if (players[channel].aiPw === true) {
                    ...
                    let aiPw = aiPwRec[param.waveId];
                    if (aiPw) {
                        // 获取当前小节对应的智能强度，如果当前没有 就取第一小节的强度
                        let ap = aiPw[param.stageIdx] || aiPw['0'];
                        let subPw = pw - ap;
                        if (pw && aiPw && subPw > 0) {
                            pw = ap;
                        }
 ...
 * @param {*} channel 
 */
function getStages(wave, channel) {
    let stages = wave.stages;
    if (wave.channelType === '1') {
        stages = wave[channel].stages;
    }
    return stages;
}
...
function anlySongEnd(song, param, channel) {
    if (song.type) {
        if (song.type === 'end') {
            ...
            if (param.pw) {
                // 取反恢复电源强度
                let p = players[channel].pw + (-param.pw);
                // 删除runner的电源强度参数
                _.unset(param, 'pw');
                // 设置电源增量
        ...
        if (song.waveEnd === true || song.waveStart === true) {
            // 波形结束后重置波形图像参数
...
function powerNext(channel, param) {
    let type = parseInt(players[channel].autoPwType || '0');
...
function playNext(channel) {
    ...
    let type = parseInt(player.playType || '0');
...
export default {
    ...
    setPw(channel, p) {
        if (channel) {
            ...
            // // 手动调节的电源强度才记录智能强度
            // if (p > 0 && p < 100 && players[channel].aiPw == true) {
            //     setAipwInfo(channel);
            // }
